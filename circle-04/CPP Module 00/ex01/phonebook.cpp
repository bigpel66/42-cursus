...
void PhoneBook::contact_add(void) {
  ...
  if (_o)
    std::cout << RED
      << "Contact will be OVERWRITTEN due to the FULLNESS!" << EOC << std::endl;
...
void PhoneBook::_detail(void) const {
  ...
  if (!opt)
    return;
  std::cout << std::endl
    << GREEN << "1. First Name : " << EOC
    << _c[opt - 1].get_f_name() << std::endl
    << GREEN << "2. Last Name : " << EOC
    <<_c[opt - 1].get_l_name() << std::endl
    << GREEN << "3. Nickname : " << EOC
    << _c[opt - 1].get_alias() << std::endl
    << GREEN << "4. Phone Number : " << EOC
    << _c[opt - 1].get_phone() << std::endl
    << GREEN << "5. Darkest Secret : " << EOC
...
void PhoneBook::contact_search(void) const {
  ...
  std::cout << std::setfill('=') << std::setw(51) << EOC << std::endl << "|"
    << std::setfill(' ') << std::setw(PADD_SIZE)
    << "index" << "|"
    << std::setfill(' ') << std::setw(PADD_SIZE)
    << "first name" << "|"
    << std::setfill(' ') << std::setw(PADD_SIZE)
    ...
    << std::setfill(' ') << std::setw(PADD_SIZE)
    << "nickname" << "|" << std::endl
    << std::setfill('=') << std::setw(51) << EOC
    << std::endl;
  for (int i = 0 ; i < size ; ++i) {
    std::cout << "|" << YELLOW
      << std::setfill(' ') << std::setw(PADD_SIZE) << i + 1
      << EOC << "|"
      << std::setfill(' ') << std::setw(PADD_SIZE)
      << _cut(_c[i].get_f_name()) << "|"
      << std::setfill(' ') << std::setw(PADD_SIZE)
      << _cut(_c[i].get_l_name()) << "|"
      << std::setfill(' ') << std::setw(PADD_SIZE)
      << _cut(_c[i].get_alias()) << "|" << std::endl
      << std::setfill('=') << std::setw(51) << EOC
      << std::endl;
  }
  _detail();
